...
read_plates <- function(file_name = NULL, sheet = 1, ncol_ = 24, nrow_ = 16){
  ...
  if(ext == "XLSX"){
    data_ <- data.frame(lapply(read.xlsx(file_name, colNames = F, rowNames = F, sheet = sheet), as.character), stringsAsFactors=F)
...
QC_statistics <- function()
{
  ...
  qcstat = do.call(rbind, lapply(1:dim((data_))[1], function(i){
    ...
    pos = na.omit(plate_table$rawIntensity[productUp=="BZCL" & plate_table$DCol!=24]); neg = na.omit(plate_table$rawIntensity[productUp=="DMSO"]);
...
if(registered == !0){
  ...
  if(!exists("custom_vers")){
    
    warning(raw_data_path);
    
    #Create the needed data folders
    unzip(raw_data_path, exdir="./www/Data",junkpaths=T) 

    if(is.null(annoframe$Content)){
      ProductUpper <- toupper(annoframe[,"ProductName"]); annoframe$Content <- "sample"
      annoframe$Content[ProductUpper == "BZCL"] <- "pos"; annoframe$Content[ProductUpper == "DMSO"] <- "neg";
      annoframe$Content[ProductUpper %in% c("CELLS","EMPTY","") | is.na(ProductUpper)] <- "cells";
    }
    
    warning("Hi5;")
    
    annoframe$Content[is.na(annoframe$Content)] = ""
    annoframe=annoframe[complete.cases(annoframe$DWell) & (!annoframe$DWell %in% ""),]	
    colnames(annoframe)[grepl("conc", colnames(annoframe))] <- "Concentration"
    
    # if DRow and DCol are not given
    #if((is.null(annoframe$DRow))||(!is.null(annoframe$DCol))){
    annoframe$DCol <- gsub("[[:alpha:]]", "", annoframe$DWell);
    annoframe$DRow <- gsub("[[:digit:]]", "", annoframe$DWell);
    #}
    
    warning("Hi1;")
    
    infoframe =  as.data.frame(openxlsx::read.xlsx(experiment_info_path, cols=1:10, colNames = !1), stringsAsFactors = !1)
    infoframe[is.na(infoframe)] <- ""; headerframe = infoframe[1:5, 1:2]
    
    warning("Hi2;")
    
    #filter out missing
    infoframe = infoframe[match("ID",infoframe[,1]):nrow(infoframe),]
    infoframe <- infoframe[!apply(infoframe,1,function(x)all(x=="")),]
    colnames(infoframe) <- as.character(as.matrix(infoframe[1,])); infoframe <- infoframe[-1,]
    infoframe <- infoframe[!(infoframe$File_name == "" | is.na(infoframe$File_name)),]
    
    warning("Hi3;")
    
    #remove spaces/replace brackets..
    infoframe$screen_id <- gsub("^\\s+|\\s+$", "", as.character(infoframe$screen_id))
    infoframe$screen_id <- gsub("/(|/)|%|[[:space:]]",".",infoframe$screen_id)
    rownames(infoframe) = paste(make.names(infoframe$screen_id),infoframe$Plate,sep="|")
    
    warning(as.character(infoframe$File_name))
    
    screen_table <- createeset(as.character(infoframe$File_name),annoframe,infoframe)
    screen_table <- screen_table[!is.na(screen_table$Plate),]
    
    warning("Hi5;")
    
    dir.create( "./www/RDA", showWarnings=T, recursive=T )
    save(infoframe,file ="./www/RDA/Info.rda")
    save(headerframe,file ="./www/RDA/Header_Info.rda")
  }
  
  # fix wrong content
  screen_table[which(toupper(screen_table$ProductName) != "BZCL" & toupper(screen_table$Content) == "POS"), "Content"] <- "empty"
  screen_table[which(toupper(screen_table$ProductName) != "DMSO" & toupper(screen_table$Content) == "NEG"), "Content"] <- "empty"
  # saveRDS(screen_table, "screen_table3.RDS")

} else
{
  screen_table <- annot;
  screen_table$Column  <- screen_table$DCol <- as.numeric(gsub("[[:alpha:]]", "", screen_table$DWell));
  screen_table$Row <- screen_table$DRow <- gsub("[[:digit:]]", "", screen_table$DWell);
  screen_table$Row <- as.factor(screen_table$Row);
  dir.create( "./www/RDA", showWarnings=T, recursive=T )
  # screen_table <- readRDS("screen_table3.RDS")
  #browser()
}

qcstat <- QC_statistics(); # save QC.rda
# believe t the data
screen_table$believe = paste0(screen_table$screen_id, "_", screen_table$Plate);
believe_ = sapply(screen_table$believe, function(i){stat_ = qcstat[i == qcstat$Plate,]; ifelse(sum(stat_$Z_Prime < .1,stat_$Signal_Vs_BG < 1.7,stat_$Out_To_In_Controls < .75,stat_$Out_To_In_Controls>1.25) > 1, !1, !0)})
screen_table$believe = believe_;
save(screen_table,file ="./www/RDA/screen_table.rda")
...
PDF_QC <- function(){
  ...
  {	
    ...
    p <- ggplot2::ggplot(data=set_table, ggplot2::aes(x=DRow, y=rawIntensity,ymin =0)) + ggplot2::facet_wrap(~Plate,nrow=3,scales="free") + ggplot2::geom_point(position = ggplot2::position_jitter(h=1,w=0.3), ggplot2::aes(colour=Content,shape=Content),show_guide = T) +  ggplot2::scale_shape_manual(values=myshape) +  ggplot2::scale_colour_manual(values=mycol) + ggplot2::ggtitle(paste("Raw Intensity: Row Wise  ",screen,sep="::"))  + ggplot2::theme_bw()
    
    set_table=set_table[with(set_table, order(Plate,Column,Row)),]
    p2 <-ggplot2::ggplot(data=set_table, ggplot2::aes(x=factor(Column), y=rawIntensity,ymin =0)) + ggplot2::facet_wrap(~Plate,nrow=3,scales="free") + ggplot2::geom_point(position = ggplot2::position_jitter(h=1,w=0.3), ggplot2::aes(colour=Content,shape=Content),show_guide = T) + ggplot2::scale_shape_manual(values=myshape) +  ggplot2::scale_colour_manual(values=mycol) + ggplot2::ggtitle(paste("Raw Intensity : Column Wise",screen,sep="::")) + ggplot2::theme_bw() + ggplot2::theme(panel.grid.major=ggplot2::element_line(color="gray50"))
...
if(HTML_QC){
  ...
  bd <- do.call(paste0, mclapply(1:length(screens), function(i){
    screen = screens[i]
    set_table = screen_table[screen_table$screen_id == screen, ]
    ...
    # rewrite JSON end
    obj <- paste0(substr(JsonBar, 1, regexpr("\"showLink\":false", JsonBar)[1]), "showLink\":false}},\"evals\":[],\"jsHooks\":[]}")
  ...
  }, mc.cores = min(length(screens), num_cores)));
  ...
  for (screen in screens) {
    ...
    bd <- do.call(paste0, mclapply(1:length(plates), function(i){
      ...
      # create heat map
      plotheat <- plotly::plot_ly(data = df_, x=col, y=row, z = dt, type = "heatmap", 
        ...
        add_trace(y=outl_[,1], x=outl_[,2],  mode = "markers", text = "possible outlier", hoverinfo = "text", showlegend=F) %>%  
        ...
        layout(title = paste0("Raw data: ", plate_id, "<br>"), margin = list(t = 100, b = 50, l = 20, r = 10), 
               xaxis = list(title = "", tickmode = "array", side = "top", ticklen = 1, linecolor = "#ffffff", linewidth = 4, 
                            tickvals = seq.int(1, ncol(raw_datamat_corrected), by = 1), tickfont = list(family = "serif", size = 10), 
               ...
               yaxis = list(title = "", tickmode = "array", ticklen = 1, linecolor = "#ffffff", linewidth = 4, 
                            tickvals = seq.int(1, nrow(raw_datamat_corrected), by = 1), tickfont = list(family = "serif", size = 10), 
                            ticks = "outside", ticktext = rev(LETTERS[1:nrow(raw_datamat_corrected)])))
      #htmltools::save_html(as.widget(plotheat), file = "test2.html")
      
      # create JSON
      JsonHeat <- htmlwidgets:::toJSON(plotly::as.widget(plotheat))[1]
  ...
  for (screen in screens) {
    ...
    bd <- do.call(paste0, mclapply(1:length(plates), function(i){
      ...
      plb$data[[4]]$marker$opacity = 0.5
      
      for (i in 1:length(plb$data)) {
        plb$data[[i]]$name = substr(plb$data[[i]]$name, 2, regexpr(",", plb$data[[i]]$name)[1] - 1)  #(cells,cells) to cells
        pltCell = plate_table[plate_table$Content == plb$data[[i]]$name, ]
        
        j = 1:length(plb$data[[i]]$text)
        plb$data[[i]]$text[j] = paste0("(", pltCell$Column[j], ", ", pltCell$Row[j], ")<br>", "Product name: ", pltCell$ProductName[j], "<br>", "Product id: ", 
                                       pltCell$ProductId[j], "<br>", "Raw Intensity: ", pltCell$rawIntensity[j], "<br>", "Concentration: ", pltCell$Concentration[j])
      }
      
      plb$layout$xaxis$title = "plate row"; plb$layout$yaxis$title = "raw intensity"; plb$layout$xaxis$titlefont$size = 12; plb$layout$yaxis$titlefont$size = 12; plb$layout$annotations[[1]]$x = 1.07; plb$layout$annotations[[1]]$y = 0.955
      # plb$layout$annotations[[1]]$text = 'Content:'
      
      JsonHeat <- htmlwidgets:::toJSON(plotly::as.widget(plb))[1]
  ...
  for (screen in screens) {
    ...
    bd <- do.call(paste0, mclapply(1:length(plates), function(i){
      ...
      plb$layout$xaxis$title = "plate column";plb$layout$yaxis$title = "raw intensity";plb$layout$xaxis$titlefont$size = 12; plb$layout$yaxis$titlefont$size = 12; plb$layout$annotations[[1]]$x = 1.07; plb$layout$annotations[[1]]$y = 0.955
      # plb$layout$annotations[[1]]$text = 'Content:'
  ...
  for (screen in screens) {
    ...
    bd <- do.call(paste0, mclapply(1:length(plates), function(i){
      ...
      for (i in 1:3) {
        ...
        plb$data[[i]]$text[j] = paste0("(", pltCell$Column[j], ", ", pltCell$Row[j], ")<br>", "Product name: ", pltCell$ProductName[j], "<br>", "Raw Intensity: ", 
                                       pltCell$rawIntensity[j], "<br>")
      }
      
      for (i in 4:length(plb$data)) {
        plb$data[[i]]$hoverinfo = "y"
        plb$data[[i]]$text = ""
      }
...
allscreens = unique(screen_table$screen_id); numberofscreens = length(allscreens); 
#all_plates = unname(list.files("./Data")); numberofplates = length(all_plates);
all_drugs = unique(screen_table[!(screen_table$ProductId %in% c("","BzCl","empty","dmso", "DMSO","cells", "NA", NA)), "ProductId"]); 
numberofdrugs = length(all_drugs) 
...
listToJSON <- list(numberofscreens = numberofscreens, numberofplates = numberofplates, numberofdrugs = numberofdrugs, doneQC = T, QCmetrics = h2table)
jsonL <- rjson::toJSON(listToJSON)
write(jsonL, "./www/Results/HTMLreport/QC.json")
#rm(list= ls()[!(ls() %in% c('report_name','writeReport','saving_libpath','DSS_typ','DSRT3', 'DSRT_set', 'DBQCCF', 'viab_path', 'dmethod_'))])
